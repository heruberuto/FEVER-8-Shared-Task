...
class AVeriTeCEvaluator:
    ...
    def evaluate_veracity(self, src, tgt):
        ...
        f1 = {
            ...
            for i, x in enumerate(
                sklearn.metrics.f1_score(tgt_labels, src_labels, labels=self.verdicts, average=None)
            )
        }
    ...
    def evaluate_questions_only(self, srcs, tgts):
        ...
        for src, tgt in zip(srcs, tgts):
            if "evidence" not in src:
                ...
                src_questions = self.extract_full_comparison_strings(src, is_target=False)[
                    : self.max_questions
                ]
            else:
                src_questions = [qa["question"] for qa in src["evidence"][: self.max_questions]]
    ...
    def get_n_best_qau(self, srcs, tgts, n=3):
        ...
        examples = [
            (
                (srcs[i]["questions"] if "questions" in srcs[i] else srcs[i]["string_evidence"]),
                tgts[i]["questions"],
                all_utils[i],
            )
            for i in idxs
...
def evaluate_veracity(prediction_file="data_store/dev_veracity_prediction.json", label_file="data/dev.json"):
    ...
    type_scores = scorer.evaluate_averitec_veracity_by_type(predictions, references, threshold=0.25)
